...
class VLMGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._buffered_inputs = [[None] * args.gradient_accumulation_steps for _ in range(3)]
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                # if i == 0:
                #     output_reward_func = [x * 2 for x in output_reward_func]
        ...
        self._metrics[f"step{inputs[0]['current_step']}_completion_length"].append(completion_length)
        ...
        for i, reward_func in enumerate(self.reward_funcs):
            ...
            if reward_func_name.endswith("_step"):
                reward_func_name = reward_func_name[:-5]
            reward_func_name = f"step{inputs[0]['current_step']}_" + reward_func_name
            self._metrics[f"rewards/{reward_func_name}"].append(reward_per_func[i].item())

        self._metrics[f"step{inputs[0]['current_step']}_reward"].append(self.accelerator.gather_for_metrics(rewards).mean().item())

        self._metrics[f"step{inputs[0]['current_step']}_reward_std"].append(self.accelerator.gather_for_metrics(std_grouped_rewards).mean().item())
        ...
        return {
            ...
            "current_step": inputs[0]['current_step'],
        }

    def compute_loss_per_step(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if return_outputs:
        ...
        if self.state.global_step % self.num_iterations == 0:
            ...
            del inputs["current_step"]
            self._buffered_inputs[current_step - 1][self._step % self.args.gradient_accumulation_steps] = inputs
        else:
            current_step = inputs[0]['current_step']
            inputs = self._buffered_inputs[current_step - 1][self._step % self.args.gradient_accumulation_steps]
        self._step += 1
        ...
        if self.beta > 0:
            ...
            self._metrics[f"step{current_step}_kl"].append(self.accelerator.gather_for_metrics(mean_kl).mean().item())
        ...
        self._metrics[f"step{current_step}_clip_ratio"].append(self.accelerator.gather_for_metrics(clip_ratio).mean().item())

        return loss, completion_ids, completion_mask

    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        # num_step = sum(1 for d in inputs[0]["prompt"] if d.get("role") == "user")
        # step_pre = next(i for i, d in enumerate(inputs[0]["prompt"]) if d.get("role") == "user")
        num_step = 3

        total_loss = 0.0
        pattern = r"<think>.*?</think>\s*<answer>(.*?)</answer>"

        completion_text_steps = []
        answer_dict = [{} for _ in range(len(inputs))]
        conv_lists = [[] for _ in range(len(inputs))]
        for step_i in range(num_step):
            inputs_step = copy.deepcopy(inputs)
            for idx in range(len(inputs_step)):
                inputs_step[idx]["current_step"] = step_i + 1
                if step_i == 0:
                    conv_lists[idx].extend(inputs[idx]["prompt"])
                else:
                    # answer
                    completion_text = completion_text_steps[step_i - 1][idx]
                    match = re.search(pattern, completion_text, re.DOTALL)
                    answer_content = match.group(1) if match else ""
                    conv_ans = {
                        "role": "assistant",
                        "content": [
                            {"type": "text", "text": answer_content},
                        ],
                    }
                    conv_lists[idx].append(conv_ans)
                    # question
                    if answer_content != "":
                        answer_content = " " + answer_content
                    answer_dict[idx][f"answer_content_step{step_i + 1}"] = answer_content
                    if step_i == 1:
                        question_text = inputs_step[idx][f"input_template_step{step_i + 1}"].format(answer_dict[idx][f"answer_content_step{step_i + 1}"])
                    elif step_i == 2:
                        question_text = inputs_step[idx][f"input_template_step{step_i + 1}"].format(answer_dict[idx][f"answer_content_step{step_i}"], answer_dict[idx][f"answer_content_step{step_i + 1}"])
                    else:
                        raise ValueError
                    conv_que = {
                        "role": "user",
                        "content": [
                            {"type": "text", "text": inputs_step[idx]["question_template"].format(Question=question_text)},
                        ],
                    }
                    conv_lists[idx].append(conv_que)
                inputs_step[idx]["prompt"] = conv_lists[idx]

            loss, completion_ids, completion_mask = self.compute_loss_per_step(model, inputs_step, return_outputs, num_items_in_batch)

            total_loss = total_loss + loss

            completion_mask = completion_mask.to(dtype=torch.bool)
            completion_texts = []
            for idx, (comp_id, comp_mask) in enumerate(zip(completion_ids, completion_mask)):
                comp_text = self.processing_class.tokenizer.decode(comp_id[comp_mask])
                completion_texts.append(comp_text)
            completion_text_steps.append(completion_texts)
